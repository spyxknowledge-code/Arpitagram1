...
db.serialize(() => {
  db.run(`CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, password_hash TEXT, salt TEXT)`);
  db.run(`CREATE TABLE IF NOT EXISTS sessions (token TEXT PRIMARY KEY, user_id INTEGER, expires_at INTEGER)`);
  db.run(`CREATE TABLE IF NOT EXISTS rooms (id INTEGER PRIMARY KEY AUTOINCREMENT, room_id TEXT UNIQUE, password_hash TEXT, salt TEXT)`);
  db.run(`CREATE TABLE IF NOT EXISTS room_members (user_id INTEGER, room_id INTEGER, PRIMARY KEY (user_id, room_id))`);
  db.run(`CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, room_id INTEGER, sender_id INTEGER, encrypted_content TEXT, iv TEXT, salt TEXT, sent_at INTEGER, is_deleted INTEGER DEFAULT 0)`);
});

const getUserId = (token) => new Promise((resolve) => {
  if (!token) return resolve(null);
  db.get('SELECT user_id FROM sessions WHERE token = ? AND expires_at > ?', [token, Date.now()], (err, row) => resolve(row ? row.user_id : null));
});
...
app.get('/api/rooms/:roomId/messages', async (req, res) => {
  ...
  db.get('SELECT id FROM rooms WHERE room_id = ?', [roomId], (err, roomRow) => {
    ...
    db.all(`SELECT m.*, u.username as sender_name FROM messages m JOIN users u ON m.sender_id = u.id WHERE m.room_id = ? AND m.is_deleted = 0 ORDER BY m.sent_at ASC`, [roomRow.id], (err, rows) => res.json(rows));
  });
});
...
io.on('connection', (socket) => {
  ...
  db.get('SELECT username FROM users WHERE id = ?', [userId], (err, row) => { socket.data.username = row ? row.username : 'User'; });
  ...
  socket.on('typing', ({ roomId, isTyping }) => { socket.to(roomId).emit('typing', { username: socket.data.username, isTyping }); });

  socket.on('room-message', async ({ roomId, encrypted_content, iv, salt, sentAt }) => {
    db.get('SELECT id FROM rooms WHERE room_id = ?', [roomId], (err, roomRow) => {
      if (!roomRow) { socket.emit('error', 'Room not found'); return; }
      db.run(`INSERT INTO messages (room_id, sender_id, encrypted_content, iv, salt, sent_at) VALUES (?, ?, ?, ?, ?, ?)`, [roomRow.id, userId, encrypted_content, iv, salt, sentAt], function(err) {
  ...
  socket.on('edit-message', async ({ messageId, roomId, encrypted_content, iv, salt }) => {
    db.run(`UPDATE messages SET encrypted_content = ?, iv = ?, salt = ? WHERE id = ? AND sender_id = ?`,
      [encrypted_content, iv, salt, messageId, userId], function(err) {
        if (err) { console.error(err); return; }
  ...
  socket.on('disconnect', () => { console.log(`❌ ${socket.data.username} disconnected`); });
});
